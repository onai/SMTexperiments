...
N_SCALLS_ALLOF = 10 # max # of s_calls in an allof

COST_CEIL_MAX = 200 # maximum entry in the cost_ceil field for an allof

MAX_N_ALLOFS = 1000 # maximum number of allofs to include in the system
MAX_N_ALLOFS_PER_COMMIT = 100 # maximum number of allofs to include in the system
MAX_INSTANCES_PER_S_CALL = 100 # maximum instance count per service call
MAX_N_COMMITS = 1000
...
def build_allof(service_calls):
    ...
    is_requests_ints = np.random.binomial(1, 0.5, n_calls)
    instances = []
...
if __name__ == '__main__':
    ...
    for i in range(n_allofs):
        ...
        with open(dest_filename, 'w') as handle:
            s_calls = build_service_calls_list(n=100)
